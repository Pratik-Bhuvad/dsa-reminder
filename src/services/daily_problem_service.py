...
from ..storage.history_repository import get_history, add_history, clear_history
...
def get_daily_problems(file_path):
    ...
    unattempted_problems = [problem for problem in available_problems if problem['id'] not in attempted_problem]
    ...
    # Select a problem to show to the user (for simplicity, we select the first unattempted problem)
